...
import('materialize-css/dist/js/materialize.min.js').then(({ default: M }) => {
    ...
    if (document.readyState === 'loading') {
      ...
      document.addEventListener('DOMContentLoaded', function() {
          console.log('==> DOMContentLoaded event fired, initializing all MaterializeCSS components now');
          initializeMaterializeCSS(M);
      });
  } else {
      // Document is already loaded
      console.log('==> Document already loaded, initializing all MaterializeCSS components now');
      initializeMaterializeCSS(M);
  }
}).catch(err => {
    ...
    <nav style="position: fixed; top: 0px">
      <div class="nav-wrapper">
        <a href="#" class="brand-logo left"
          >VisAnatomy: A Chart Corpus with Fine-Grained Semantic Labels</a
        >
...
function initializeMaterializeCSS(M){
  if (M) {
    M.AutoInit();
    console.log('==> ALL MaterializeCSS components initialized successfully, calling init() now')
